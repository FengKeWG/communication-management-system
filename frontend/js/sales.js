...
function fetchSalesData() {
    ...
    const resultCountDiv = document.getElementById('sales-search-result-count');
    contentDiv.innerHTML = '<p>正在加载业务员列表...</p>';
    if (resultCountDiv) resultCountDiv.style.display = 'none';
    let queryParams = [];
    if (currentSalesGeneralSearch) queryParams.push(`query=${encodeURIComponent(currentSalesGeneralSearch)}`);
    if (currentSalesNameSearch) queryParams.push(`name=${encodeURIComponent(currentSalesNameSearch)}`);
    if (currentSalesEmailSearch) queryParams.push(`email=${encodeURIComponent(currentSalesEmailSearch)}`);
    if (currentSalesClientCountSearch) queryParams.push(`client_count=${encodeURIComponent(currentSalesClientCountSearch)}`);
    ...
    fetch(`/api/fetch_sales${queryString}`)
        ...
        .then(data => {
            ...
            const hasSearchTerms = currentSalesGeneralSearch || currentSalesNameSearch || currentSalesEmailSearch || currentSalesClientCountSearch;
...
function generateSalesTable(output, totalCount) {
    ...
    const headers = ['ID', '姓名', '性别', '生日', '邮箱', '电话', '客户数', '操作'];
    ...
    lines.forEach(line => {
        ...
        let genderText = '未知';
        if (genderValueFromBackend === '1' || genderValueFromBackend?.toLowerCase() === 'male' || genderValueFromBackend === '男') {
            genderText = '男';
        } else if (genderValueFromBackend === '2' || genderValueFromBackend?.toLowerCase() === 'female' || genderValueFromBackend === '女') {
            genderText = '女';
        }
    ...
    tableRows.forEach(row => {
        const salesId = row.dataset.id;
        const actionCell = row.querySelector('.action-cell');
        if (salesId && actionCell) {
            let buttonsHTML = `<button class="view-btn icon-btn" onclick="viewSalesDetails('${salesId}')" title="查看详情">
...
function performSalesSearch() {
    ...
    currentSalesEmailSearch = document.getElementById('salesEmailSearchInput').value.trim();
...
function clearSalesSearch() {
    ...
    currentSalesGeneralSearch = '';
    currentSalesNameSearch = '';
    currentSalesEmailSearch = '';
    currentSalesClientCountSearch = '';
    const detailedArea = document.getElementById('detailed-sales-search-area');
    const toggleBtn = document.getElementById('toggleDetailedSalesSearchBtn');
